...
class AddNewStudentControllerImp extends AddNewStudentController {
  ...
  Future<void> saveFileToLocal(XFile file) async {
    file.saveTo("C:/Users/Omar-Bkdash/Desktop/${file.name}");
  }
  ...
  void initListAndController() {
    firstName = TextEditingController(text: student?.firstName ?? 'Omar');
    lastName = TextEditingController(text: student?.lastName ?? 'Bkdash');
    dateOfBirth =
        TextEditingController(text: student?.dateOfBirth ?? '1993-01-01');
    placeOfBirth =
        TextEditingController(text: student?.placeOfBirth ?? 'New York');
    parentName = TextEditingController(text: student?.parentName ?? 'Suliman');
    email =
